...
addParamValue(p,'cbar_position','southoutside'); % Position of the colorbar. Works only for 4-D plots: Will not have any effect for 3-D plot
...
addParamValue(p,'D4_colormap_flipud','on')      % Inverse the colormap (colour scheme) for the 4th dimension. Works only for 4-D plots: Will not have any effect for 3-D plot
...
addParamValue(p,'light_orientation',[45 45]);   % Orientation of light source. First value is the azimuth (0=pointing at first row; 90=pointing at last column; 180=pointing at last row; 270=pointing at first column), second one is the height (given as an angle, as seen from the middle of the image)
...
addParamValue(p,'title_size',14);                % Font size of the title
...
elseif strcmp(p.Results.extra_dimension,'')==0
    ...
    if strcmp(p.Results.D4_colormap_flipud,'off')==1 && strcmp(p.Results.bed_colormap_flipud,'off')==1
     ...
     %   colormap([1,1,1;redblue(round(p.Results.cbar_colors));eval(strcat(p.Results.bed_colormap,'(',num2str(p.Results.bed_colors),')'))]);
    elseif strcmp(p.Results.D4_colormap_flipud,'on')==1 && strcmp(p.Results.bed_colormap_flipud,'off')==1
        ...
        % colormap([1,1,1;flipud(redblue(round(p.Results.cbar_colors)));eval(strcat(p.Results.bed_colormap,'(',numstr(p.Results.bed_colors),')'))]);
    elseif strcmp(p.Results.D4_colormap_flipud,'off')==1 && strcmp(p.Results.bed_colormap_flipud,'on')==1
       ...
       %  colormap([1,1,1;redblue(round(p.Results.cbar_colors));flipud(eval(strcat(p.Results.bed_colormap,'(',num2str(p.Results.bed_colors),')')))]);
   elseif strcmp(p.Results.D4_colormap_flipud,'on')==1 && strcmp(p.Results.bed_colormap_flipud,'on')==1
       ...
       %  colormap([1,1,1;flipud(redblue(round(p.Results.cbar_colors)));flipud(eval(strcat(p.Results.bed_colormap,'(',num2str(p.Results.bed_colors),')')))]);
   end
