...
def barometric_p(P,dh,T):
    a = -lapse_rate*dh/(1000*T)    #note lapse rate in atmospheric is +ve, hence change sign in all formulas, convert to per m
...
for i in range (num_points):
     expected_paths.append(f'{root}/All_glaciers/{Id}/ERA5L_at_gridpoint/{i}.parquet') #create list of expected filepaths
...
if True:
    ...
    for lon,lat in zip(lons,lats):   #iterate for each point
        ...
        delta_T = ((h_dem-h_ERA5L)/1000)*lapse_rate   #Change in Temps due to lapse rates
        ...
        df_out['t2m'] = df_out['t2m']+delta_T    #apply T lapse rate  
        ...
        df_out.to_parquet(f'{root}/All_glaciers/{Id}/ERA5L_at_gridpoint/{index}.parquet',index=False)
        index+=1 
        #print(index)
    print(time.time()-t1)
